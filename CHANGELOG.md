...
- Tag pushes run a dedicated **Release** workflow: embedded-UI binaries for Linux (amd64, arm64, armv7), macOS (amd64, arm64), and Windows (amd64); `sha256sums.txt`; GitHub Release notes auto-generated by GitHub; multi-arch container image on GHCR (including `latest` only for stable tags). **CI** on `main` pushes a single-platform image tagged `main` and `sha-*` only — it does not move `latest`, so arm users keep a multi-arch `:latest` from the last stable release.
